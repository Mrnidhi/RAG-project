        ...
        if form.is_valid():
            ...
            try:
                ...
                try:
                    ...
                    # Store session data for future questions
                    request.session['qdrant_collection'] = "sample_transtest"
                    request.session['qdrant_url'] = qdrant_url
                    request.session['texts_count'] = len(texts)  # Store text count for validation
    ...
    if request.method == "POST":
        ...
        if query:
            try:
                ...
                texts_count = request.session.get('texts_count', 0)
                
                print(f"Session data - Collection: {collection_name}, URL: {qdrant_url}, Texts: {texts_count}")
                
                if not collection_name or not qdrant_url or texts_count == 0:
                ...
                # Create Qdrant client with better error handling
                try:
                    ...
                    print(f"Qdrant client error: {client_error}")
                    return render(request, 'ask_question.html', {"error": f"Database connection error: {str(client_error)}"})
                ...
                try:
                    ...
                    print(f"Vector store recreation error: {vs_error}")
                    return render(request, 'ask_question.html', {"error": f"Vector store error: {str(vs_error)}"})
